					...
					struct buffer_head **p)
{
	...
	*p = NULL;
	...
	//根据inode号和每个组的inode个数(保存在超级块中s_inodes_per_group的成员)，
	//就可以计算出这个inode所在的组(全部的组描述符已经在ext2_fill_super()函数中读入内存，并初始化完毕)，
	 ...
	 */
	//根据inode编号，组中保存inode的首块号(保存在组描述符中的bg_inode_table成员)，以及块大小和inode大小，
	//计算出包含这个inode的块号和块内偏移
...
//从磁盘上读取一个指定的Inode，然后设置VFS的Inode和Ext2文件系统的内存Inode结构
void ext2_read_inode (struct inode * inode)
{
	//根据VFS inode获取 ext2_inode_info 结构
	struct ext2_inode_info *ei = EXT2_I(inode);
	ino_t ino = inode->i_ino;
	struct buffer_head * bh;
	//请求磁盘驱动程序读取指定的inode结构，保存在raw_inode中
	...
	if (IS_ERR(raw_inode))
 		goto bad_inode;
	//根据raw_inode设置VFS inode和 Ext2内存inode
	...
	//设置i_op和i_fop指针
	//从磁盘格式的角度来看，目录和文件是没有区别的，但是VFS提供给上层的程序接口是区分文件和目录的
	//所以ext2_read_inode要根据文件的类别为i_op设置不同的inode_operations
