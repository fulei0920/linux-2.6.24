...
void mnt_set_mountpoint(struct vfsmount *mnt, struct dentry *dentry, struct vfsmount *child_mnt)
{
	...
	//旧的dentry实例的d_mounted值加1，这样内核能够识别出有一个文件系统装载在这里
	dentry->d_mounted++;
}
 ...
 */
//将新的vfsmount实例添加到全局散列表以及父文件系统vfsmount实例中的子文件系统链表
...
static int attach_recursive_mnt(struct vfsmount *source_mnt, struct nameidata *nd, struct nameidata *parent_nd)
{
	...
	else 
	{
		...
		//而mnt_mountpoint成员指向装载点在父文件系统中的dentry实例
...
static int graft_tree(struct vfsmount *mnt, struct nameidata *nd)
{
	...
	if (S_ISDIR(nd->dentry->d_inode->i_mode) != S_ISDIR(mnt->mnt_root->d_inode->i_mode))
		return -ENOTDIR;
	...
	if (IS_ROOT(nd->dentry) || !d_unhashed(nd->dentry))
		err = attach_recursive_mnt(mnt, nd, NULL);//添加到父文件系统的命名空间
out_unlock:
 ...
 */
//负责处理共享、从属和不可绑定装载，
//它可以改变装载标志或在涉及的各个vfsmount实例之间建立所需的数据结构的关联
 ...
 * do loopback mount.
 */
//用于通过回环接口(lookback interface)装载一个文件系统
 ...
 */
//处理一些必须的锁定操作，并确保一个文件系统不会重复装载到同一位置
//(当然，将同一文件系统多次装载到不同位置是可能的)
