...
//全局数组check_part包含了一些函数指针，能够识别特定的分区类型。
//在标准的计算机上，通常会使用PC Bios或邋錏FI分区，但对更神秘的类型如SGI Ultrix或Acorn Cumana分区也提供了支持。
//这些函数中的每一个都允许察看该块设备(这意味着用disk_add注册设备时，必须已经能够从该设备读取数据)，如果检测到某种知道的分区方案，
//check_part中的函数会向rescan_partitions返回该信息。
...
void add_partition(struct gendisk *disk, int part, sector_t start, sector_t len, int flags)
{
	...
	//与完整磁盘相比，ktype不是ktype_block，而设置为ktype_part。
	//这使得可以区分源自磁盘和源自分区的uevent
	p->kobj.ktype = &ktype_part;
	kobject_init(&p->kobj);
	//用kobject_add添加新对象，使之成为块设备子系统的一个成员，
	//因此提供有关该分区信息的sysfs项会出现在/sys/block中
	kobject_add(&p->kobj);
