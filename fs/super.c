 ...
 */
//获取一个super_block对象
//首先在链表中查找指定的super_block对象是否存在，如果找不到，就重新分配一个新的
	...
	int (*fill_super)(struct super_block *, void *, int), struct vfsmount *mnt)
{
	...
	if (s->s_root) //说明这个块设备已经mount过了，那么内存中已经有现成的super_block的完整信息，不需要进过磁盘IO操作来设置了
	{
...
//调用特定于文件系统的get_sb函数读取相关的超级块，并返回struct super_block的实例
