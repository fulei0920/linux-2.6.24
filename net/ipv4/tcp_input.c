...
static int tcp_check_dsack(struct tcp_sock *tp, struct sk_buff *ack_skb, struct tcp_sack_block_wire *sp, int num_sacks, u32 prior_snd_una)
{
	...
	//dup_sack -- ±íÃ÷½ÓÊÕÕßÊÕµ½ÖØ¸´µÄÊý¾Ý
	//undo_marker < end_seq <= prior_snd_una -- ±íÃ÷¸ÃÖØ¸´µÄÊý¾ÝÔÚµ±Ç°(undo_marker, prior_snd_una)ÖØ´«·¶Î§Ö®ÄÚ£¬
	//ËµÃ÷½øÐÐÁË²»±ØÒªµÄÖØ´«, ÍøÂçÓµÈû¿ÉÄÜ²»ÑÏÖØ£¬¼õÉÙÖØ´«undo_retrans¼ÆÊý£¬Èç¹ûundo_retrans½µµ½0£¬¾ÍËµÃ÷Ö®Ç°µÄÖØ´«¶¼ÊÇ²»±ØÒªµÄ£¬½øÐÐÓµÈûµ÷Õû³·Ïú¡£ 
...
tcp_sacktag_write_queue(struct sock *sk, struct sk_buff *ack_skb, u32 prior_snd_una)
{
	...
	if (!tp->sacked_out)			//Èç¹ûÖ®Ç°Ã»ÓÐSACKedµÄÊý¾Ý
	{
		if (WARN_ON(tp->fackets_out))
			tp->fackets_out = 0;    	//FACKÊÇ¸ù¾Ý×îÐÂµÄSACKÀ´¼ÆËãµÄ£¬sacked_outÎªÁã£¬Ôò fackets_outÒ²±Ø¶¨ÎªÁã
	...
	for (i = 0; i < num_sacks; i++) 
	{
		...
		*/
		...
		tcp_for_write_queue_from(skb, sk)
		{
			...
			//ÒòÎªÖØ´«¶ÓÁÐÊÇÅÅÐòµÄ£¬Òò´Ëµ±Ç°SKBµÄÆðÊ¼ÐòºÅ´óÓÚµ±Ç°´¦ÀíµÄSACKµÄ½áÊøÐòºÅÊ±£¬
			//ËµÃ÷µ±Ç°SKB²»ÔÚ´ËSACK·¶Î§Ö®ÄÚ£¬±¾´ÎÑ­»·²»±ØÔÙ×ö´¦Àí
			...
			//Èç¹ûÊÇDSACK£¬ÇÒ¸ÃTCP¶Î±»SACK(²¿·Ö»òÈ«²¿)£¬Í¬Ê±¸ÃTCP¶ÎµÄ¼Ç·ÖÅÆÓÐÖØ´«±ê¼Ç£¬
			...
			/* The frame is ACKed. */
			//Èç¹û´ËSKBÒÑ¾­È·ÈÏ¹ý£¬ÔòÌø¹ý¸ÃSKB£¬´¦ÀíºóÐøµÄSKB
			if (!after(TCP_SKB_CB(skb)->end_seq, tp->snd_una))
			{
				if (sacked & TCPCB_RETRANS)   
				{
					if ((dup_sack && in_sack) && (sacked & TCPCB_SACKED_ACKED))  
						reord = min(fack_count, reord); //ÖØ´«£¬±»ÖØ¸´È·ÈÏ¡£¡£¡£
				}
...
static int tcp_clean_rtx_queue(struct sock *sk, s32 *seq_rtt_p, int prior_fackets)
{
	...
	while ((skb = tcp_write_queue_head(sk)) && skb != tcp_send_head(sk))
	{
		...
		{
			/* Èç¹ûÃ»ÓÐÊ¹ÓÃTSO£¬»òseq >= snd_una£¬ÄÇÃ´¾ÍÍË³ö±éÀú*/  
			//¸Ã¶ÎÃ»ÓÐ±»È·ÈÏ
		...
		if (sacked)
		{
			...
			{
				ca_seq_rtt = now - scb->when;   /* Í¨¹ý´ËACK¼ÆËãskbµÄRTT²ÉÑùÖµ*/  
