...
//ÔÚ²»Ö§³ÖSACKÊ±£¬ÎªÓÉÓÚÁ¬½Ó½ÓÊÕµ½ÖØ¸´È·ÈÏ¶ø½øÈë¿ìËÙ»Ø¸´½×¶ÎµÄÖØ¸´È·ÈÏÊýãÐÖµ¡£
...
static int tcp_check_dsack(struct tcp_sock *tp, struct sk_buff *ack_skb, struct tcp_sack_block_wire *sp, int num_sacks, u32 prior_snd_una)
{
	...
	//ÅÐ¶ÏdsackµÄÊý¾ÝÊÇ·ñÒÑ¾­±»ackÍêÈ«È·ÈÏ¹ýÁË£¬Èç¹ûÈ·ÈÏ¹ýÁË£¬ÎÒÃÇ¾ÍÐèÒª¸üÐÂundo_retransÓò£¬Õâ¸öÓò±íÊ¾ÖØ´«µÄÊý¾Ý¶ÎµÄ¸öÊý¡£ 
...
tcp_sacktag_write_queue(struct sock *sk, struct sk_buff *ack_skb, u32 prior_snd_una)
{
	...
	if (!tp->sacked_out)			//Èç¹ûÖ®Ç°Ã»ÓÐSACKedµÄÊý¾Ý
	{
		if (WARN_ON(tp->fackets_out))
			tp->fackets_out = 0;    	//FACKÊÇ¸ù¾Ý×îÐÂµÄSACKÀ´¼ÆËãµÄ£¬Ò²ÒªÎª0 
	 ...
	 */
	//Èç¹ûSACKÐÅÏ¢ÊÇºÜÔçÒÔÇ°µÄ£¬Ö±½Ó¶ªÆú
	...
	/* ÅÐ¶ÏÊÇ·ñ½øÈë¿ìËÙÂ·¾¶¡£ 
     * ¶ÔµÚÒ»¸ö¿é£ºÖ»ÒªÇóÆðÊ¼ÐòºÅÏàÍ¬ 
     * ¶ÔÓÚ·ÇµÚÒ»¸ö¿é£ºÒªÇóÆðÊ¼ÐòºÅºÍ½áÊøÐòºÅ¶¼ÏàÍ¬ 
     * Ò²¾ÍÊÇËµ£¬¿ìËÙÂ·¾¶Ö¸µÄÊÇÖ»ÓÐµÚÒ»¸ö¿éµÄ½áÊøÐòºÅÔö¼ÓµÄÇé¿ö 
     */  
	force_one_sack = 1;
	...
	if (force_one_sack)
	{
		num_sacks = 1;	 /* ¿ìËÙÂ·¾¶Ê±Ö»ÓÐµÚÒ»¸ö¿éÓÐ±ä»¯£¬´¦ÀíµÚÒ»¸ö¿é¼´¿É */ 
	}
	else 
	{
		int j;
		tp->fastpath_skb_hint = NULL; /* ÉÏ´ÎµÚÒ»¸öSACK¿éµÄ½áÊø´¦£¬Ò²ÊÇÕâ´Î¿ìËÙÂ·¾¶µÄ¿ªÊ¼µã£¬ÂýËÙÂ·¾¶ÖÐÖØÖÃÁË */  
	...
	for (i = 0; i < num_sacks; i++) 
	{
		...
		int fack_count;
		int dup_sack = (found_dup_sack && (i == first_sack_index));  	//µ±Ç°SACKÊÇ·ñÎªDSACK¿é
		...
		/* high_seqÊÇ½øÈëRecovery»òLossÊ±µÄsnd_nxt£¬Èç¹ûhigh_seq±»SACKÁË£¬ÄÇÃ´ºÜ¿ÉÄÜÓÐÊý¾Ý°ü 
          * ¶ªÊ§ÁË£¬²»È»¾Í¿ÉÒÔACKµôhigh_seq·µ»ØOpenÌ¬ÁË¡£
		*/
		//Èç¹ûsack¶ÎµÄ½áÊøÐòÁÐºÅ´óÓÚ½«Òª·¢ËÍµÄ×î´óÐòÁÐºÅ£¬Õâ¸öÇé¿öËµÃ÷ÎÒÃÇ¿ÉÄÜÓÐÊý¾Ý¶ªÊ§¡£Òò´ËÉèÖÃ¶ªÊ§±ê¼Ç¡£ÕâÀï¿ÉÒÔ¿´µ½Ò²¾ÍÊÇÉÏÃæËùËµµÄÊÂ¼þBµ½´ï
...
static void tcp_rearm_rto(struct sock *sk)
{
	...
	///Îª0ËµÃ÷ËùÓÐµÄ´«ÊäµÄ¶Î¶¼ÒÑ¾­acked¡£´ËÊ±remove¶¨Ê±Æ÷¡£·ñÔòÖØÆô¶¨Ê±Æ÷¡£
	if (!tp->packets_out)
	{
...
static int tcp_ack_update_window(struct sock *sk, struct sk_buff *skb, u32 ack, u32 ack_seq)
{
	...
	//ÔÚSYN»òSYN/ACK±¨ÎÄ±¾ÉíµÄ´°¿Ú×Ö¶ÎÊ¼ÖÕ²»×öÈÎºÎµÄÀ©´ó
...
static int tcp_ack(struct sock *sk, struct sk_buff *skb, int flag)
{
	...
	else  /* ½øÈëÂýËÙÂ·¾¶ */  
	{
		...
		/* ¸ù¾ÝSACKÑ¡Ïî±êÖ¾ÖØ´«¶ÓÁÐÖÐSKBµÄ¼Ç·ÖÅÆ×´Ì¬*/  
		if (TCP_SKB_CB(skb)->sacked)  
			flag |= tcp_sacktag_write_queue(sk, skb, prior_snd_una);

		/* ²é¿´ACKÊÇ·ñÐ¯´øECE±êÖ¾ */
		//¿´¿´ÊÇ·ñÊÕµ½ÁËÏÔÊ¾ÓµÈûÍ¨Öª£º
		if (TCP_ECN_rcv_ecn_echo(tp, tcp_hdr(skb)))   //Â·ÓÉÆ÷Í¨Öª¶ª°ü£¬ÖÃÉÏeceÎ»
			flag |= FLAG_ECE;

		tcp_ca_event(sk, CA_EVENT_SLOW_ACK);  		 /* ÂýËÙÂ·¾¶ÓµÈûÊÂ¼þ¹³×Ó*/  
	}
	...
	if (!prior_packets)				/*¼ì²éÊÇ·ñÓÐ·¢ËÍÇÒÎ´È·ÈÏµÄ¶Î*/  
	...
	if (before(tp->frto_highmark, tp->snd_una))
	 ...
	 /* Èç¹ûACKÊÇÖØ¸´µÄ¡¢»òÕß´øÓÐSACKÑ¡Ïî¡¢»òÕß²»ÊÇOpenÌ¬*/  
	if (tcp_ack_is_dubious(sk, flag)) 
	{
		 /* Èç¹û´ËACKÊÇ¿ÉÒÉµÄ*/  
		  /* ¿ÉÒÉÊÇÖ¸ÒÑ¾­´¦ÓÚÓµÈû×´Ì¬£¬»ò¸ÕÊÕµ½ÓµÈûÐÅºÅ¡£ 
