...
int udp_sendmsg(struct kiocb *iocb, struct sock *sk, struct msghdr *msg, size_t len)
{
	...
	//当前的sock还有pending的帧(skb)没有发送到IP层*/
	...
	{	
		//如果msg中不带目的地址信息，则通常是用send调用进入，
		//在send之前调用了connect建立连接，建立连接后相应状态为TCP_ESTABLISHED
		//即使目的地址为空，但如果套接字处于连接状态，则仍然认为目的地址合法，允许继续传送数据
		if (sk->sk_state != TCP_ESTABLISHED)
		//如果既不是sendto，由不是connect+send的方式进入的话，那就有问题了
			return -EDESTADDRREQ;
		daddr = inet->daddr;
		dport = inet->dport;
	...
	//如果rt为空，即无路由信息，则先用struct flowi结构记录查找路由表的索引信息，
	//再调用 ip_route_output_flow函数查询路由表，得到路由信息。
	if (rt == NULL) 
	{
...
do_append_data:
	...
	//判断是否有cork标记(MSG_MORE)，如果没有，则说明需要立即发送，
	//则调用udp_push_pending_frames()接口发送报文，实际是将包提交至IP层；
	//如果设置了cork，则说明需要阻塞等待直到数据达到MTU大小，则完成本次的udp_sendmsg()处理。
	else if (!corkreq)
