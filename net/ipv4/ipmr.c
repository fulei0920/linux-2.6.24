...
//注册一组函数指针，这些函数指针在procfs的例程中被调用，
//用于遍历返回给用户的数据:一个函数用于初始化打印动作，
//另一个用于向前移动一次指针，还有一个用于打印一个数据项。
//这些函数会在内部保存必要的上下文信息以便了解当前的打印项在哪里，
//以及可以从哪个地方重新开始打印等。
static int ipmr_vif_open(struct inode *inode, struct file *file)
{
	return seq_open_private(file, &ipmr_vif_seq_ops, sizeof(struct ipmr_vif_iter));
}
