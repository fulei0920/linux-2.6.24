	...
	*/
	...
	while (still_changing)	
	{
		...
		for (i=1; i < chg_nr; i++)  
		{
			...
			/* or, if current=<start_addr> & last=<end_addr> & <current_addr> = <last_addr>, swap */ //?为什么这种情况下也需要交换?
			if ((change_point[i]->addr < change_point[i-1]->addr) ||
				((change_point[i]->addr == change_point[i-1]->addr) &&
				 (change_point[i]->addr == change_point[i]->pbios->addr) &&		//current=<start_addr>
				 (change_point[i-1]->addr != change_point[i-1]->pbios->addr)))	//last=<end_addr>
