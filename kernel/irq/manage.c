...
int setup_irq(unsigned int irq, struct irqaction *new)
{
	...
	*p = new;
	...
	new->dir = NULL;
	////在proc文件系统中建立目录/proc/irq/NUM/name
 ...
 */
//释放IRQ
//此函数会删除中断处理例程，而且如果没有其他设备注册在该IRQ线，就关闭该IRQ。
//注意，要识别此处理例程时，内核需要IRQ编号以及设备标识符。对共享IRQ而言，这一点尤为重要。
//dev_id -- 标识给定的设备
...
//注册IRQ
//此函数会注册一个中断处理例程。
//首先确保所请求的中断是一个有效的中断，而且还没有分配个另一个设备，除非这两个设备能够共享IRQ
