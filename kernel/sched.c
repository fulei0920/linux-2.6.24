...
struct cfs_rq
{
	struct load_weight load;//队列上所有进程的累积负荷值
	unsigned long nr_running;//队列上可运行的进程的数目

	u64 exec_clock;
	u64 min_vruntime;//队列上所有进程的最小虚拟运行时间,其是单调递增的

	struct rb_root tasks_timeline;//用于在按时间排序的红黑树中管理所有进程
	struct rb_node *rb_leftmost;//总是指向树最左边的结点，即最需要被调度的进程
...
struct rq {
	...
	struct list_head leaf_cfs_rq_list;
#endif
	struct rt_rq rt;//嵌入的子就绪队列，用于实时调度器
	 ...
	 */
	unsigned long nr_uninterruptible;//先前在运行队列链表中而现在睡眠在邋TASK_UNINTERRUPTIBLE状态的进程数量(对所有运行队列来说，只有这些字段的总数才是有意义的)

	struct task_struct *curr, *idle;//curr指向当前运行的进程的task_struct实例，idle指向空闲进程(空闲线程)的task_struct实例
	unsigned long next_balance;
	struct mm_struct *prev_mm;

	u64 clock, prev_clock_raw;//colck和prev_raw_clock用于实现就绪队列自身的时钟，该值在每个调度周期都会更新
	s64 clock_max_delta;
