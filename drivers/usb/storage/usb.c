...
//提供给usb core使用，它需要比较driver和device，从而确定设备是被这个driver所支持的
//我们只需要比较四项就可以了，因为这四项已经足以确定一个设备了，厂商/产品/序列号/
...
static int usb_stor_control_thread(void * __us)
{
	...
	for(;;) 
	{
		 ...
		 */
		else if (us->srb->device->id && !(us->flags & US_FL_SCM_MULT_TARG)) 
		{
		 ...
		 * their inquiry data */
		else if ((us->srb->cmnd[0] == INQUIRY) && (us->flags & US_FL_FIX_INQUIRY)) 
		{
...
static int get_device_info(struct us_data *us, const struct usb_device_id *id)
{
	...
	us->subclass = (unusual_dev->useProtocol == US_SC_DEVICE) ? idesc->bInterfaceSubClass : unusual_dev->useProtocol;
	us->protocol = (unusual_dev->useTransport == US_PR_DEVICE) ? idesc->bInterfaceProtocol : unusual_dev->useTransport;
	 ...
	 */
	//检查unusual_dev.h文件，看是否定义了一行没有意义的句子。
	//如果这个设备设了US_SC_DEVICE，那么其subclass将从描述符中读出来，
	//如果不然，则让subcla=unusual_dev->useProtocol，
	//但是如果后者又真的和描述符里读出来的一样，吗么这个设备就没有必要把自己的useProtocol定义在
	// unusual_devs.h 中了,因为反正也可以从描述符里读出来. 还不如和大众一样设为 US_SC_DEVICE 得了.
	if (id->idVendor || id->idProduct)
	{
		...
		if (unusual_dev->useProtocol != US_SC_DEVICE && us->subclass == idesc->bInterfaceSubClass)
			msg += 1;
		if (unusual_dev->useTransport != US_PR_DEVICE && us->protocol == idesc->bInterfaceProtocol)
			msg += 2;
...
/* First stage of disconnect processing: stop all commands and remove the host */
...
static int usb_stor_scan_thread(void * __us)
{
	...
	if (delay_use > 0) 
	{
		...
		wait_event_freezable_timeout(us->delay_wait, test_bit(US_FLIDX_DISCONNECTING, &us->flags), delay_use * HZ);
	}
	...
	if (!test_bit(US_FLIDX_DISCONNECTING, &us->flags)) 
	{
		...
		if (us->protocol == US_PR_BULK && !(us->flags & US_FL_SINGLE_LUN)) 
		{
			mutex_lock(&us->dev_mutex);
...
static int storage_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
	...
	if (!host) 
	{
		printk(KERN_WARNING USB_STORAGE "Unable to allocate the scsi host\n");
		return -ENOMEM;
	}
	...
	if (result) 
	{
		printk(KERN_WARNING USB_STORAGE "Unable to add the scsi host\n");
		goto BadDevice;
	}
