		 ...
		 void *data, u16 size, int timeout)
{
	...
	US_DEBUGP("%s: rq=%02x rqtype=%02x value=%04x index=%02x len=%u\n",
			__FUNCTION__, request, requesttype, value, index, size);
...
int usb_stor_clear_halt(struct us_data *us, unsigned int pipe)
{
	...
	result = usb_stor_control_msg(us, us->send_ctrl_pipe, USB_REQ_CLEAR_FEATURE, 
		USB_RECIP_ENDPOINT, USB_ENDPOINT_HALT, endp, NULL, 0, 3*HZ);
...
static int usb_stor_bulk_transfer_sglist(struct us_data *us, unsigned int pipe,
		struct scatterlist *sg, int num_sg, unsigned int length, unsigned int *act_len)
{
	int result;
		...
		void *buf, unsigned int length_left, int use_sg, int *residual)
{
	...
	if (use_sg)
	{
		...
		result = usb_stor_bulk_transfer_sglist(us, pipe, (struct scatterlist *) buf, use_sg, length_left, &partial);
		length_left -= partial;
	}
	else 
	{
...
int usb_stor_Bulk_max_lun(struct us_data *us)
{
	...
	result = usb_stor_control_msg(us, us->recv_ctrl_pipe, US_BULK_GET_MAX_LUN, 
		USB_DIR_IN | USB_TYPE_CLASS | USB_RECIP_INTERFACE, 0, us->ifnum, us->iobuf, 1, HZ);
...
int usb_stor_Bulk_transport(struct scsi_cmnd *srb, struct us_data *us)
{
	...
	US_DEBUGP("Bulk Command S 0x%x T 0x%x L %d F %d Trg %d LUN %d CL %d\n",
			le32_to_cpu(bcb->Signature), bcb->Tag, le32_to_cpu(bcb->DataTransferLength),
			bcb->Flags, (bcb->Lun >> 4), (bcb->Lun & 0x0F),  bcb->Length);
	...
	//transfer_length 可能为 0,因为有的命令它并不需要传输数据,所以它没有数据阶段
	if (transfer_length)
	{
	...
	else if (bcs->Signature != us->bcs_signature)
	{
		US_DEBUGP("Signature mismatch: got %08X, expecting %08X\n",
			  le32_to_cpu(bcs->Signature), le32_to_cpu(us->bcs_signature));
		return USB_STOR_TRANSPORT_ERROR;
	}
 ...
 */
static int usb_stor_reset_common(struct us_data *us, u8 request, u8 requesttype, u16 value, u16 index, void *data, u16 size)
{
	int result;
	int result2;
	...
	if (result < 0) 
	{
 	...
 	wait_event_interruptible_timeout(us->delay_wait, test_bit(US_FLIDX_DISCONNECTING, &us->flags), HZ*6);
	if (test_bit(US_FLIDX_DISCONNECTING, &us->flags)) 
	{
...
int usb_stor_Bulk_reset(struct us_data *us)
{
	...
	return usb_stor_reset_common(us, US_BULK_RESET_REQUEST, USB_TYPE_CLASS | USB_RECIP_INTERFACE, 0, us->ifnum, NULL, 0);
}
