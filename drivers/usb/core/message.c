...
{ 
	...
	if (!wait_for_completion_timeout(&ctx.done, expire)) 
	{
		...
		dev_dbg(&urb->dev->dev, "%s timed out on ep%d%s len=%d/%d\n", current->comm,
			usb_endpoint_num(&urb->ep->desc), usb_urb_dir_in(urb) ? "in" : "out",
...
int usb_get_status(struct usb_device *dev, int type, int target, void *data)
{
	...
	ret = usb_control_msg(dev, usb_rcvctrlpipe(dev, 0), USB_REQ_GET_STATUS, 
		USB_DIR_IN | type, 0, target, status, sizeof(*status), USB_CTRL_GET_TIMEOUT);
...
void usb_disable_device(struct usb_device *dev, int skip_ep0)
{
	...
	//将设备里所有端点给 disable 掉
	dev_dbg(&dev->dev, "%s nuking %s URBs\n", __FUNCTION__, skip_ep0 ? "non-ep0" : "all");
	 ...
	 */
	//一是将设备当前配置使用的每个接口都从系统里给 unregister掉，也就是将接口和它对应的驱动给分开
	if (dev->actconfig) 
	{
 ...
 */
static void usb_enable_interface(struct usb_device *dev, struct usb_interface *intf)
{
	struct usb_host_interface *alt = intf->cur_altsetting;
	int i;
...
int usb_set_interface(struct usb_device *dev, int interface, int alternate)
{
	...
	ret = usb_control_msg(dev, usb_sndctrlpipe(dev, 0),USB_REQ_SET_INTERFACE, 
		USB_RECIP_INTERFACE, alternate, interface, NULL, 0, 5000);
	...
	for (i = 0; (i < USB_MAXIADS && config->intf_assoc[i]); i++) 
	{
		...
		if (inum >= first_intf && inum <= last_intf)
		{
			if (!retval)
				...
				dev_err(&dev->dev, "Interface #%d referenced by multiple IADs\n", inum);
		}
	}
...
free_interfaces:
	...
	if ((ret = usb_control_msg(dev, usb_sndctrlpipe(dev, 0), USB_REQ_SET_CONFIGURATION, 0, 
 			configuration, 0, NULL, 0, USB_CTRL_SET_TIMEOUT)) < 0) 
	{
	...
	dev->actconfig = cp;
	// cp 有三种可能为空，一是参数 configuration 为-1，
	//一是参数 configuration 为 0 ，且从设备的 config 数组里拿出来的就为空，
	//一是SET_CONFIGURATION 请求除了问题
	if (!cp) 
	{
		usb_set_device_state(dev, USB_STATE_ADDRESS);
	...
	for (i = 0; i < nintf; ++i) 
	{
		...
		//获得这个接口的 0 号设置
		//spec 里指明接口的默认设置总是 0 号设置，所以这里的目的就是获得接口的默认设置
		alt = usb_altnum_to_altsetting(intf, 0);
		 ...
		 * then I wouldn't trust its reply anyway.
		 */
		//如果没有拿到设置 0，拿 altsetting 数组里的第一项来充数
		if (!alt)
			alt = &intf->altsetting[0];
