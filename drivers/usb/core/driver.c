...
static int usb_create_newid_file(struct usb_driver *usb_drv)
{
	...
	if (usb_drv->probe != NULL)
		error = sysfs_create_file(&usb_drv->drvwrap.driver.kobj, &driver_attr_new_id.attr);
exit:
	return error;
}
...
static void usb_remove_newid_file(struct usb_driver *usb_drv)
{
	...
	if (usb_drv->probe != NULL)
		sysfs_remove_file(&usb_drv->drvwrap.driver.kobj, &driver_attr_new_id.attr);
}
...
static const struct usb_device_id *usb_match_dynamic_id(struct usb_interface *intf, struct usb_driver *drv)
{
	struct usb_dynid *dynid;
...
static int usb_resume_interface(struct usb_interface *intf, int reset_resume)
{
	...
	if (interface_to_usbdev(intf)->state == USB_STATE_NOTATTACHED || is_active(intf))
		goto done;
...
static int autosuspend_check(struct usb_device *udev, int reschedule)
{
	...
	if (udev->actconfig) 
	{
		for (i = 0; i < udev->actconfig->desc.bNumInterfaces; i++) 
		{
			...
			if (intf->pm_usage_cnt > 0)
				return -EBUSY;
			if (intf->needs_remote_wakeup && !udev->do_remote_wakeup)
			{
	...
	if (reschedule) 
	{
		...
		{
			queue_delayed_work(ksuspend_usb_wq, &udev->autosuspend, round_jiffies_relative(suspend_time - j));
		}
		return -EAGAIN;
	}
	return 0;
	...
	if (udev->state == USB_STATE_SUSPENDED) 
	{
		...
		if (parent) 
		{
			...
			if (status == 0) 
			{
				...
				if (status || udev->state == USB_STATE_NOTATTACHED)
				{
					usb_autosuspend_device(parent);
...
{
	struct usb_device *udev = container_of(work, struct usb_device, autosuspend.work);
