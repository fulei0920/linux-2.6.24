...
//在引导系统的时候,可以为kernel指定参数.如果配置了”nousb”,就明确禁止使用USB
 ...
 */
//struct usb_host_config结构里的数组interface并不一定是按照接口号的顺序存储的，
//你必须使用 usb_ifnum_to_if()来获得指定接口号对应的struct usb_interface结构体
 ...
 */
//接口里的altsetting数组也不一定是按照设置编号来顺序存储的，
//你必须使用usb_altnum_to_altsetting()来获得接口里的指定设置
...
struct usb_interface *usb_find_interface(struct usb_driver *drv, int minor)
{
	...
	retval = driver_for_each_device(&drv->drvwrap.driver, NULL, &argb, __find_interface);
...
struct usb_device * usb_alloc_dev(struct usb_device *parent, struct usb_bus *bus, unsigned port1)
{
	...
	else 
	{
		...
		if (parent->devpath[0] == '0') //设备连接在root hub上
...
static int __init usb_init(void)
{
	...
	if (retval)
		goto out;
	//注册USB总线，只有成功的将USB总线子系统注册到系统中，我们才可以向这个总线添加 USB 设备
	retval = bus_register(&usb_bus_type);
	if (retval) 
		goto bus_register_failed;
	//执行 host controller 相关的初始化
	//一个实际的总线也是一个设备，必须单独注册，因为 USB 是通过快速串行通信来读写数据，这里把它当作了字符设备来注
	retval = usb_host_init();
	if (retval)
		goto host_init_failed;
	//usbfs 相关的初始化
	//usbfs为咱们提供了在用户空间直接访问usb硬件设备的接口
	...
	//注册 USB device driver
	//USB device driver 对应的是整个设备
	//USB driver 对应某个接口
