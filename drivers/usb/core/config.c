...
static int usb_parse_endpoint(struct device *ddev, int cfgno, int inum,
    int asnum, struct usb_host_interface *ifp, int num_ep, unsigned char *buffer, int size)
{
	unsigned char *buffer0 = buffer;
	...
	//端点描述符与配置描述符、接口描述符不一样，它是可能有两种大小的
	...
	//这里的端点号不能为 0， 因为端点0是没有描述符的， 也不能大于16
	if (i >= 16 || i == 0) 
	{
		dev_warn(ddev, "config %d interface %d altsetting %d has an "
	...
	if (d->bInterval < i || d->bInterval > j)
	{
		dev_warn(ddev, "config %d interface %d altsetting %d "
		    "endpoint 0x%X has an invalid bInterval %d, changing to %d\n",
		    cfgno, inum, asnum, d->bEndpointAddress, d->bInterval, n);
...
skip_to_next_endpoint_or_interface_descriptor:
	i = find_next_descriptor(buffer, size, USB_DT_ENDPOINT, USB_DT_INTERFACE, NULL);
	return buffer - buffer0 + i;
}
		...
		unsigned char *buffer, int size, u8 inums[], u8 nalts[])
{
	...
	for ((i = 0, alt = &intfc->altsetting[0]); i < intfc->num_altsetting; (++i, ++alt)) 
	{
		...
		{
			dev_warn(ddev, "Duplicate descriptor for config %d interface %d altsetting %d, skipping\n", cfgno, inum, asnum);
			goto skip_to_next_interface_descriptor;
		}
	}
	...
	alt->extra = buffer;
	//这里 find_next_descriptor 的 dt1 参数和 dt2 参数就不再一样了，
	//因为如果一个接口只用到端点 0，它的接口描述符后边儿是不会跟有端点描述符的。
	i = find_next_descriptor(buffer, size, USB_DT_ENDPOINT, USB_DT_INTERFACE, &n);
	alt->extralen = i;
	if (n > 0)
...
skip_to_next_interface_descriptor:
	i = find_next_descriptor(buffer, size, USB_DT_INTERFACE, USB_DT_INTERFACE, NULL);
	return buffer - buffer0 + i;
}

//使用 GET_DESCRIPTOR请求时，得到的数据并不是杂乱无序的，而是有规可循的，
//一般来说，配置描述符后面跟的是第一个接口的接口描述符，接着是这个接口里第一个端点的端点描述符，
//如果有 class-和 vendor-specific 描述符的话，会紧跟在对应的标准描述符后面，
//不管接口有多少端点有多少都是按照这个规律顺序排列。当然有些厂商会特立独行一些，非要先返回第二个接口
    ...
    struct usb_host_config *config, unsigned char *buffer, int size)
{
	...
	int len, retval;
	//inums每一项记录一个接口编号
	//nalts每一项记录inums对应的接口编号的接口所拥有的设置数目
	...
	{
		dev_warn(ddev, "config %d has too many interfaces: %d, using maximum allowed: %d\n",
	...
	for ((buffer2 = buffer, size2 = size); size2 > 0; (buffer2 += header->bLength, size2 -= header->bLength)) 
	{
		...
		//如果这是个接口描述符就说明这个配置的某个接口拥有一个设置，是没有什么所谓的设置描述符的 ， 
		//一个接口描述符就代表了存在一个设置， 接口描述里的bInterfaceNumber 会指出这个设置隶属于哪个接口。
		//那么这里除了是接口描述符还有可能是什么？还有可能是 class-和 vendor-specific 描述符。
		if (header->bDescriptorType == USB_DT_INTERFACE)
		{
			...
			if (d->bLength < USB_DT_INTERFACE_SIZE) 
			{
				dev_warn(ddev, "config %d has an invalid interface descriptor of length %d, skipping\n", cfgno, d->bLength);
				continue;
			}
			...
			/* Have we already encountered this interface? Count its altsettings */
			for (i = 0; i < n; ++i) 
			{
				if (inums[i] == inum)
					break;
		...
		{
			dev_warn(ddev, "config %d contains an unexpected descriptor of type 0x%X, skipping\n",
	...
	if (n != nintf)
		dev_warn(ddev, "config %d has %d interface%s, different from the descriptor's value: %d\n", cfgno, n, plural(n), nintf_orig);
	else if (n == 0)
	...
	for (i = 0; i < nintf; ++i)
	{
		...
		{
			dev_warn(ddev, "too many alternate settings for config %d interface %d: %d, "
...
int usb_get_configuration(struct usb_device *dev)
{
	...
	for (; cfgno < ncfg; cfgno++) 
	{
		...
		if (result < length) 
		{
			dev_warn(ddev, "config index %d descriptor too short (expected %i, got %i)\n", cfgno, length, result);
			length = result;
		}
