...
//iso_packets --在等时传输的方式下指定你需要传输多少个包,对于非等时模式,这个参数直接使用0
//表示的是struct urb 结构最后那个变长数组iso_frame_desc的元素数目，也就是应该与number_of_packets的值相同，
//所以对于控制/中断/批量传输，这个参数都应该为 0
//mem_flags -- 申请内存的方式
...
int usb_submit_urb(struct urb *urb, gfp_t mem_flags)
{
	...
	//在设备进入 Configured 状态之前，主机只能使用控制传输，通过缺省管道，也就是管道 0 来和设备进行交流
	...
	if (max <= 0) 
	{
		dev_dbg(&dev->dev, "bogus endpoint ep%d%s in %s (bad maxpacket %d)\n",
			usb_endpoint_num(&ep->desc), is_out ? "out" : "in", __FUNCTION__, max);
		return -EMSGSIZE;
	}
