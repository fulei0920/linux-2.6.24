...
struct usb_hub 
{
	...
	unsigned long		change_bits[1];	/* ports with logical connect
							status change */
	unsigned long		busy_bits[1];	/* ports being reset or resumed */
...
static int get_hub_descriptor(struct usb_device *hdev, void *data, int size)
{
	...
	for (i = 0; i < 3; i++) 
	{
		ret = usb_control_msg(hdev, usb_rcvctrlpipe(hdev, 0), USB_REQ_GET_DESCRIPTOR, 
			USB_DIR_IN | USB_RT_HUB, USB_DT_HUB << 8, 0, data, size, USB_CTRL_GET_TIMEOUT);
...
{
	return usb_control_msg(hdev, usb_sndctrlpipe(hdev, 0), 
		USB_REQ_SET_FEATURE, USB_RT_PORT, feature, port1, NULL, 0, 1000);
}
...
{
	int status = set_port_feature(hub->hdev, (selector << 8) | port1, USB_PORT_FEAT_INDICATOR);
	if (status < 0)
		dev_dbg (hub->intfdev,
...
static int hub_configure(struct usb_hub *hub, struct usb_endpoint_descriptor *endpoint)
{
	...
	{
		case 0: //full/low speed 的hub 的 bDeviceProtocol 是 0,这种 hub 就没有 tt. 
			break;
		case 1: //对于 high speed 的 hub,其 bDeviceProtocol 为 1 表示是 single tt 的 
			dev_dbg(hub_dev, "Single TT\n");
			hub->tt.hub = hdev;
			break;
		case 2: //对于 high speed 的 hub,其 bDeviceProtocol 为 2 表示是 multiple tt 的
...
void usb_set_device_state(struct usb_device *udev, enum usb_device_state new_state)
{
	...
	else if (new_state != USB_STATE_NOTATTACHED) 
	{
		...
		if (udev->parent) 
		{
			if (udev->state == USB_STATE_SUSPENDED || new_state == USB_STATE_SUSPENDED)
				;	/* No change to wakeup settings */
			else if (new_state == USB_STATE_CONFIGURED)
				device_init_wakeup(&udev->dev,(udev->actconfig->desc.bmAttributes & USB_CONFIG_ATT_WAKEUP));
			else
...
static int hub_port_wait_reset(struct usb_hub *hub, int port1, struct usb_device *udev, unsigned int delay)
{
	int delay_time, ret;
	u16 portstatus;
	u16 portchange;
...
check_highspeed (struct usb_hub *hub, struct usb_device *udev, int port1)
{
	...
	status = usb_get_descriptor (udev, USB_DT_DEVICE_QUALIFIER, 0, qual, sizeof *qual);
...
hub_power_remaining (struct usb_hub *hub)
{
	...
	for (port1 = 1; port1 <= hdev->maxchild; ++port1) 
	{
		...
		if (delta > hub->mA_per_port)
			dev_warn(&udev->dev, "%dmA is over %umA budget for port %d!\n", delta, hub->mA_per_port, port1);
		remaining -= delta;
	}
	if (remaining < 0) {
...
static void hub_port_connect_change(struct usb_hub *hub, int port1, u16 portstatus, u16 portchange)
{
	...
	if (!(portstatus & USB_PORT_STAT_CONNECTION)) 
	{
		...
		if ((wHubCharacteristics & HUB_CHAR_LPSM) < 2 && !(portstatus & (1 << USB_PORT_FEAT_POWER)))
 		...
 		//如果该端口还处于 ENABLE 的状态,把这个端口给 disable掉
...
//判断每个端口是否有变化,如果有变化就去处理它,没有变化也没有关系,
//接下来判断是否 hub 整体上有变化,如果有有变化,那么也去处理它
static void hub_events(void)
{
	...
	while (1) 
	{
		...
		dev_dbg(hub_dev, "state %d ports %d chg %04x evt %04x\n",
				hdev->state, hub->descriptor? hub->descriptor->bNbrPorts: 0,
		...
		for (i = 1; i <= hub->descriptor->bNbrPorts; i++)
		{
			...
			if (hub->activating && !hdev->children[i-1] && (portstatus & USB_PORT_STAT_CONNECTION))
				connect_change = 1;
			...
			//表明连在该端口的设备的 suspend 状态有变化,
			//并且是从suspended状态出来,也就是说resume完成
			if (portchange & USB_PORT_STAT_C_SUSPEND)  
			...
			//说明这个端口可能曾经存在电流过大的情况,而现在这种情况不存在了,
			//或者本来不存在而现在存在了
			//有一种比较特别的情况是,如果其它的端口电流过大,那么将会导致本端口断电,
			//即 hub 上一个端口出现 over-current 条件将有可能引起 hub
			//上其它端口陷入 powered off 的状态. 
			if (portchange & USB_PORT_STAT_C_OVERCURRENT)
			{
				dev_err (hub_dev, "over-current change on port %d\n", i);
				clear_port_feature(hdev, i, USB_PORT_FEAT_C_OVER_CURRENT);
				hub_power_on(hub); //重新上电
			}

			//端口从Resetting 状态进入到 Enabled 状态
 ...
 * lock.
 */
int usb_reset_composite_device(struct usb_device *udev, struct usb_interface *iface)
{
	int ret;
	...
	if (config) 
	{
		...
		for (i = 0; i < config->desc.bNumInterfaces; ++i)
		{
			...
			if (cintf != iface)
				down(&cintf->dev.sem);
			if (device_is_registered(&cintf->dev) && cintf->dev.driver) 
			{
