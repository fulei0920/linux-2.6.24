...
static int i2c_register_adapter(struct i2c_adapter *adap)
{
	...
	if (adap->dev.parent == NULL) 
	{
		...
		pr_debug("I2C adapter driver [%s] forgot to specify physical device\n", adap->name);
	}
...
int i2c_attach_client(struct i2c_client *client)
{
	...
	if (adapter->client_register) 
	{
		...
		{
			dev_dbg(&adapter->dev, "client_register failed for client [%s] at 0x%02x\n", client->name, client->addr);
		}
	}
...
out_list:
	list_del(&client->list);
	dev_err(&adapter->dev, "Failed to attach i2c client %s at 0x%02x (%d)\n", client->name, client->addr, res);
out_unlock:
	mutex_unlock(&adapter->clist_lock);
	return res;
}
...
s32 i2c_smbus_read_i2c_block_data(struct i2c_client *client, u8 command, u8 length, u8 *values)
{
	...
	data.block[0] = length;
	if (i2c_smbus_xfer(client->adapter,client->addr,client->flags,
		I2C_SMBUS_READ,command,I2C_SMBUS_I2C_BLOCK_DATA,&data))
		return -1;
...
s32 i2c_smbus_write_i2c_block_data(struct i2c_client *client, u8 command, u8 length, const u8 *values)
{
	...
	memcpy(data.block + 1, values, length);
	return i2c_smbus_xfer(client->adapter, client->addr, client->flags,
			      I2C_SMBUS_WRITE, command, I2C_SMBUS_I2C_BLOCK_DATA, &data);
}
                                   ...
                                   union i2c_smbus_data * data)
{
	...
	case I2C_SMBUS_BLOCK_DATA:
		...
		else
		{
			...
			{
				dev_err(&adapter->dev, "smbus_access called with invalid block write size (%d)\n", data->block[0]);
				return -1;
			}
