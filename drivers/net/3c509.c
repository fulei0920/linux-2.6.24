...
el3_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	netif_stop_queue (dev); //停止设备队列，因此能禁止内核提交后续的传输请求
	...
	dev->trans_start = jiffies;
	//检查该设备的内存是否有足够的空间容纳一个1536个字节的包。
	//如果有，驱动程序就会启动队列，允许内核再次提交传输请求；
	//否则就会指示设备(通过用一个outw调用写一个配置寄存器)，当条件满足时产生一个中断。
	//中断处理例程将使用netif_start_queue重启设备队列，使内核能够重新继续传输
