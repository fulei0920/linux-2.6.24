/*
	...
	Contributors:
		...
		Tobias Ringstr枚m - Rx interrupt status checking suggestion
...
static int __devinit rtl8139_init_board (struct pci_dev *pdev, struct net_device **dev_out)
{
	...
	*dev_out = NULL;
	...
	dev_printk (KERN_DEBUG, &pdev->dev, "unknown chip version, assuming RTL-8139\n");
...
static int rtl8139_open (struct net_device *dev)
{
	...
	void __iomem *ioaddr = tp->mmio_addr;
	//注册这个设备的中断处理函数
	//中断号也是BIOS在初始化阶段分配并写入设备的配置空间的，
	//然后Linux在建 立pci_dev时从配置空间读出这个中断号然后写入pci_dev的irq成员中，
	//所以我们注册中断程序需要中断号就是直接从pci_dev里取就可以了
	retval = request_irq (dev->irq, rtl8139_interrupt, IRQF_SHARED, dev->name, dev);
	if (retval)
		return retval;
	//分配发送和接收的缓存空间
	//根据官方文档，发送一个数据包的过程是这样的：先从应用程序中把数据包拷贝到一段连续的内存中（这段内存就是我们这里要分配的缓存），
	//然后把这段内存的地 址写进网卡的数据发送地址寄存器(TSAD)中,这个寄存器的偏移量是TxAddr0 = 0x20。
	//在把这个数据包的长度写进另一个寄存器（TSD）中，它的偏移量是TxStatus0 = 0x10。
	//然后就把这段内存的数据发送到网卡内部的发送缓冲中(FIFO),最后由这个发送缓冲区把数据发送到网线上。
	//tx_bufs是发送缓冲内存的首地址，rx_ring是接收缓存内存的首地址，他们都是虚拟地址，
	//tx_bufs_dma和rx_ring_dma均是这一段内存的物理地址。
	//同一个事物，既用虚拟地址来表示它还要用物理地址呢，是这样的， 
	//CPU执行程序用到这个地址时，用虚拟地址，而网卡设备向这些内存中存取数据时用的是物理地址
	tp->tx_bufs = dma_alloc_coherent(&tp->pci_dev->dev, TX_BUF_TOT_LEN, &tp->tx_bufs_dma, GFP_KERNEL);
	tp->rx_ring = dma_alloc_coherent(&tp->pci_dev->dev, RX_BUF_TOT_LEN, &tp->rx_ring_dma, GFP_KERNEL);
