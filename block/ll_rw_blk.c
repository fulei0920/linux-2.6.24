...
//内核提供的标准函数blk_init_queue_node，用于产生一个标准的请求队列。
//在这种情况下，驱动程序自身唯一必须提供的管理函数就是request_fn。
//任何其他的管理问题都通过标准函数处理。
//用这种方法实现请求管理的驱动程序，在调用add_disk激活磁盘之前，
//需要调用blk_init_queue_node创建请求队列，并将结果request_queue实例附加到设备的gendisk实例
