...
struct kobject 
{
	const char		* k_name;//对象的文本名称，可利用sysfs导出到用户空间
	struct kref		kref;//引用计数
	struct list_head	entry;//标准的链表元素，用于将若干kobject放置到一个链表中(在这种情况下称为集合)
	struct kobject		* parent;//一个指向父对象的指针，可用于在kobject之间建立层次结构
	struct kset		* kset;//将对象与其它对象放置到一个集合时，则需要kset
	struct kobj_type	* ktype;//提供了包含kobject的数据结构的更多详细信息。其中，最重要的是用于释放该数据结构资源的析构器函数
...
//kobj_type提供了与sysfs文件系统的接口。
//如果多个对象通过该文件系统导出类似的信息，则可以简化，使多个对象共享同一个ktype来提供所需的方法
struct kobj_type 
{
...
struct kset 
{
	...
	struct kobject		kobj;
	struct kset_uevent_ops	*uevent_ops;//提供了若干函数指针，用于将集合的状态信息传递给用户层
};
